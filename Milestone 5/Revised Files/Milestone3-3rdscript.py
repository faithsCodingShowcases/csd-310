...
cursor.execute("SELECT p.product_name, p.year,p.unit_price,s.company_name FROM product p INNER JOIN supplier s ON p.supplier_id=s.supplier_id WHERE p.year < 2019")
products = cursor.fetchall()

for product in products:
    print("Product Name: {}\n Product Creation Year: {}\n Price: {}\n Company Name: {}\n".format(product[0], product[1], product[2], product[3]))
